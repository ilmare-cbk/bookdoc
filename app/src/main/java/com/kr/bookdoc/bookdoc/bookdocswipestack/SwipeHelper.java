...
public class SwipeHelper implements View.OnTouchListener {
    ...
    private void checkViewPosition() {
        ...
        float parentFirstThirdForHeight = mSwipeStack.getHeight() / 3f;
        float parentLastThirdForHeight = parentFirstThirdForHeight * 2;
        ...
        } else if (viewCenterVertical < parentFirstThirdForHeight &&
                mSwipeStack.getAllowedSwipeDirections() != SwipeStack.SWIPE_DIRECTION_ONLY_UP) {
            swipeViewToUp(mAnimationDuration / 2);
        } else if (viewCenterVertical > parentLastThirdForHeight &&
                mSwipeStack.getAllowedSwipeDirections() != SwipeStack.SWIPE_DIRECTION_ONLY_DOWN) {
            swipeViewToDown(mAnimationDuration / 2);
        }  else {
            resetViewPosition();
        }
    }
    ...
    private void swipeViewToUp(int duration){
        if (!mListenForTouchEvents) return;
        mListenForTouchEvents = false;
        mObservedView.animate().cancel();
        mObservedView.animate()
                ...
                .setListener(new AnimationUtils.AnimationEndListener() {
                    @Override
                    public void onAnimationEnd(Animator animation) {
                        mSwipeStack.onViewSwipedToUp();
                    }
                });
    }

    private void swipeViewToDown(int duration){
        if (!mListenForTouchEvents) return;
        mListenForTouchEvents = false;
        mObservedView.animate().cancel();
        mObservedView.animate()
                ...
                .setListener(new AnimationUtils.AnimationEndListener() {
                    @Override
                    public void onAnimationEnd(Animator animation) {
                        mSwipeStack.onViewSwipedToDown();
                    }
                });
    }
